...
class _ServerPageState extends State<ServerPage> {
  ...
  void initState() {
    ...
    Future.delayed(Duration(milliseconds: 500), () async {
      // 1. 先尝试启用输入控制权限（INJECT_EVENTS）
      if (!gFFI.serverModel.inputOk) {
        debugPrint("应用启动：自动请求输入控制权限");
        await parent.target?.invokeMethod("start_input");
      }
      
      // 2. 然后请求屏幕录制权限并启动服务（MediaProjection）
      if (!gFFI.serverModel.isStart) {
        debugPrint("应用启动：自动请求屏幕录制权限并启动服务");
        // 直接调用startService而不是toggleService，避免显示确认对话框
