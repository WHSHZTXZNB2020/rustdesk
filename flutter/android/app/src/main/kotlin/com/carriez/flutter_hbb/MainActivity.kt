                ...
                "start_input" -> {
                    if (InputService.ctx == null) {
                        if (checkInjectEventsPermission(this)) {
                            try {
                                InputService(this)
                                Log.d(logTag, "已有INJECT_EVENTS权限，直接初始化InputService")
                                Companion.flutterMethodChannel?.invokeMethod(
                                    "on_state_changed",
                                    mapOf("name" to "input", "value" to InputService.isOpen.toString())
                                )
                            ...
                            Log.d(logTag, "请求INJECT_EVENTS权限")
                            Log.i(logTag, "INJECT_EVENTS权限请求已发送")
                            requestInjectEventsPermission(this) { granted ->
                                if (granted) {
                                    ...
                                    Log.i(logTag, "INJECT_EVENTS权限被拒绝")
                                }
                                activity.runOnUiThread {
                                    Companion.flutterMethodChannel?.invokeMethod(
                                        "on_state_changed",
                        ...
                        Log.d(logTag, "InputService已经运行中")
                        Companion.flutterMethodChannel?.invokeMethod(
                            "on_state_changed",
                            mapOf("name" to "input", "value" to InputService.isOpen.toString())
                        )
